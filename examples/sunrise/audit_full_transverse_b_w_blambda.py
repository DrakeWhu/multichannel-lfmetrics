#!/usr/bin/env python3
"""Audit full-transverse B, W_perp, and wavelength-averaged B for a publication case.

Read-only post-processing helper for SUNRISE publication smokes.
It reads WarpX/openPMD field diagnostics and the final-bunch trajectory NPZ,
then writes PNG/CSV/JSON diagnostics without modifying simulation outputs.
"""
...
def render_step(
    *,
    ...
    print(
        ...
        f"dB_laser={row['delta_B_to_laserB_axis_deg']:7.3f}deg"
    )
    ...
    unit_quiver(
        ...
        threshold=max(0.05 * np.nanmax(btotal), 2.0e3),
    )
    ...
    unit_quiver(
        ...
        wperp,
        stride=quiver_stride,
        threshold=max(0.05 * np.nanmax(wperp), 0.03e12),
    )
    ...
    unit_quiver(
        ...
        blambda,
        stride=quiver_stride,
        threshold=max(0.05 * np.nanmax(blambda), 0.2e3),
    )
    ...
    fig.suptitle(
        f"SOFT iter_022 c003 — full transverse fields, step {step}, t/T={row['t_over_T_800nm']:.2f}",
        fontsize=12,
    )

    png = output_dir / f"full_transverse_B_W_Blambda_step{step:06d}.png"
...
def write_outputs(
    *,
    ...
    json_path = output_dir / "full_transverse_B_W_Blambda_audit.json"
    json_path.write_text(json.dumps(summary, indent=2) + "\n", encoding="utf-8")

    csv_path = output_dir / "full_transverse_B_W_Blambda_table.csv"
    with csv_path.open("w", newline="", encoding="utf-8") as stream:
        writer = csv.DictWriter(stream, fieldnames=list(rows[0].keys()))
        writer.writeheader()
        writer.writerows(rows)

    tT = np.asarray([row["t_over_T_800nm"] for row in rows], dtype=float)

    fig, axes = plt.subplots(2, 3, figsize=(14.5, 7), constrained_layout=True)

    axes[0, 0].plot(tT, [row["Btotal_p99_kT"] for row in rows], marker="o", label="raw B")
    axes[0, 0].plot(tT, [row["Bresidual_p99_kT"] for row in rows], marker="o", label="W/c")
    axes[0, 0].plot(tT, [row["Blambda_p99_kT"] for row in rows], marker="o", label=r"$\langle B\rangle_\lambda$")
    axes[0, 0].set_title("p99 magnetic-like scale [kT]")
    axes[0, 0].set_xlabel(r"$t/T$")
    axes[0, 0].legend()

    axes[0, 1].plot(tT, [row["Wperp_p99_TV_m"] for row in rows], marker="o")
    axes[0, 1].set_title(r"$W_{\perp,p99}$ [TV/m]")
    axes[0, 1].set_xlabel(r"$t/T$")

    axes[0, 2].plot(tT, [row["Bresidual_p99_kT"] for row in rows], marker="o")
    axes[0, 2].set_title(r"$(W_\perp/c)_{p99}$ [kT]")
    axes[0, 2].set_xlabel(r"$t/T$")

    axes[1, 0].plot(tT, [row["delta_B_to_laserB_axis_deg"] for row in rows], marker="o")
    axes[1, 0].axhline(0.0, linestyle="--", linewidth=1)
    axes[1, 0].set_title(r"$\Delta\theta(B,\ B_{\rm laser})$")
    axes[1, 0].set_xlabel(r"$t/T$")

    axes[1, 1].plot(tT, [row["delta_B_to_hex_axis_deg"] for row in rows], marker="o", label="raw B")
    axes[1, 1].plot(tT, [row["delta_W_to_hex_axis_deg"] for row in rows], marker="o", label="W")
    axes[1, 1].plot(tT, [row["delta_Blambda_to_hex_axis_deg"] for row in rows], marker="o", label=r"$\langle B\rangle_\lambda$")
    axes[1, 1].set_title("offset to honeycomb axes [deg]")
    axes[1, 1].set_xlabel(r"$t/T$")
    axes[1, 1].legend(fontsize=8)

    axes[1, 2].plot(tT, [row["Blambda_p99_kT"] for row in rows], marker="o")
    axes[1, 2].set_title(r"$\langle B_\perp\rangle_{\lambda,p99}$ [kT]")
    axes[1, 2].set_xlabel(r"$t/T$")

    for ax in axes.flat:
        ax.grid(True, alpha=0.3)

    trend_png = output_dir / "full_transverse_B_W_Blambda_trends.png"
    fig.savefig(trend_png, dpi=180, facecolor="white")
    plt.close(fig)

    if zip_path is not None:
        files = sorted(output_dir.glob("*.png")) + sorted(output_dir.glob("*.csv")) + sorted(output_dir.glob("*.json"))
...
def main() -> None:
    ...
    lambda0_m = args.lambda0_um * 1.0e-6

    if args.output_dir.exists():
        raise FileExistsError(f"Output directory already exists: {args.output_dir}")
    if args.zip_path is not None and args.zip_path.exists():
        raise FileExistsError(f"Zip already exists: {args.zip_path}")
    ...
    summary = {
        "schema": "multichannel_full_transverse_laser_filter_audit_v3",
    ...
    write_outputs(output_dir=args.output_dir, zip_path=args.zip_path, summary=summary, rows=rows)
    ...
    print("FULL_TRANSVERSE_B_W_BLAMBDA_AUDIT=PASS")
