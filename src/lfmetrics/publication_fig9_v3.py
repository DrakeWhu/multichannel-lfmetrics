...
PUBLICATION_FIG9_SCHEMA = "multichannel_publication_fig9_snapshot_v3"
...
class PublicationFig9SnapshotResult:
    ...
    magnetic_color_min_kT: float
    magnetic_color_max_kT: float
...
def _positive_percentile_range(
    values: np.ndarray,
    lower_percentile: float,
    upper_percentile: float,
) -> tuple[float, float]:
    array = np.asarray(values, dtype=float)
    selected = array[np.isfinite(array) & (array > 0.0)]
    if selected.size == 0:
        return 0.0, 1.0

    lower = float(np.percentile(selected, lower_percentile))
    upper = float(np.percentile(selected, upper_percentile))
    if not np.isfinite(upper) or upper <= 0.0:
        upper = float(np.max(selected))
    if not np.isfinite(lower) or lower < 0.0 or lower >= upper:
        lower = 0.0
    return lower, upper
...
def _particle_scatter_style(n_background_points: int) -> tuple[float, float]:
    if n_background_points <= 5_000:
        return 3.0, 0.55
    if n_background_points <= 20_000:
        return 1.8, 0.43
    if n_background_points <= 50_000:
        return 1.1, 0.34
    return 0.75, 0.28
...
def write_publication_fig9_snapshot(
    *,
    ...
    ez_color_limit_floor_TV_m: float = 5.0,
    magnetic_visible_percentile: float = 60.0,
    magnetic_color_percentile: float = 99.0,
    magnetic_length_gamma: float = 1.8,
    energy_percentile: float = 99.0,
    field_slice_method: str = "linear",
    quiver_max_arrows_per_axis: int = 23,
    dpi: int = 220,
) -> PublicationFig9SnapshotResult:
    """Write one refined three-panel field/particle snapshot.

    ``E_z`` remains the common scalar background in all panels. In ``xy``,
    ``(B_x, B_y)`` arrows are colored with ``inferno`` by ``|B_perp|`` and use
    thresholded nonlinear length scaling. Background and tracked particles use
    the same marker size within each panel.
    """
    ...
    slab_half_width = _validated_positive_finite(
        "background_slab_half_width_m", background_slab_half_width_m
    )
    transverse_half_width = _validated_positive_finite(
        "transverse_half_width_m", transverse_half_width_m
    )
    xi_padding_back = _validated_positive_finite(
        "xi_padding_back_m", xi_padding_back_m
    )
    xi_padding_front = _validated_positive_finite(
        "xi_padding_front_m", xi_padding_front_m
    )
    ...
    magnetic_visible_pct = _validated_percentile(
        "magnetic_visible_percentile", magnetic_visible_percentile
    )
    magnetic_color_pct = _validated_percentile(
        "magnetic_color_percentile", magnetic_color_percentile
    )
    if magnetic_visible_pct >= magnetic_color_pct:
        raise ValueError(
            "magnetic_visible_percentile must be smaller than magnetic_color_percentile"
        )
    ez_floor_v_m = _validated_positive_finite(
        "ez_color_limit_floor_TV_m", ez_color_limit_floor_TV_m
    ) * 1.0e12
    magnetic_gamma = _validated_positive_finite(
        "magnetic_length_gamma", magnetic_length_gamma
    )
    if (
    ...
    ez_xy = read_mesh_plane(
        field_path, "E", "z", plane="xy", coordinate_m=center_z,
        method=field_slice_method, iteration=int(step)
    )
    ez_xz = read_mesh_plane(
        field_path, "E", "z", plane="xz", coordinate_m=center_y,
        method=field_slice_method, iteration=int(step)
    )
    ez_yz = read_mesh_plane(
        field_path, "E", "z", plane="yz", coordinate_m=center_x,
        method=field_slice_method, iteration=int(step)
    )
    bx_xy = read_mesh_plane(
        field_path, "B", "x", plane="xy", coordinate_m=center_z,
        method=field_slice_method, iteration=int(step)
    )
    by_xy = read_mesh_plane(
        field_path, "B", "y", plane="xy", coordinate_m=center_z,
        method=field_slice_method, iteration=int(step)
    )
    ...
    x_xy, y_xy, ez_xy_matrix = _plane_for_display(
        ez_xy, horizontal_axis="x", vertical_axis="y"
    )
    z_xz, x_grid, ez_xz_matrix = _plane_for_display(
        ez_xz, horizontal_axis="z", vertical_axis="x"
    )
    z_yz, y_grid, ez_yz_matrix = _plane_for_display(
        ez_yz, horizontal_axis="z", vertical_axis="y"
    )
    x_xy_bx, y_xy_bx, bx_matrix = _plane_for_display(
        bx_xy, horizontal_axis="x", vertical_axis="y"
    )
    x_xy_by, y_xy_by, by_matrix = _plane_for_display(
        by_xy, horizontal_axis="x", vertical_axis="y"
    )
    ...
    for label, candidate_x, candidate_y in (
        ...
        if not np.allclose(
            x_xy, candidate_x, rtol=1.0e-12, atol=1.0e-18
        ) or not np.allclose(
            y_xy, candidate_y, rtol=1.0e-12, atol=1.0e-18
        ):
            raise ValueError(f"E/z and {label} transverse coordinates do not match")
    ...
    ez_xy_crop = _cropped_matrix_values(
        x_xy, y_xy, ez_xy_matrix, x_limits, y_limits
    )
    ez_xz_crop = _cropped_matrix_values(
        xi_grid, x_grid, ez_xz_matrix, xi_limits, x_limits
    )
    ez_yz_crop = _cropped_matrix_values(
        xi_grid, y_grid, ez_yz_matrix, xi_limits, y_limits
    )
    ez_limit_v_m = max(
        _finite_nonzero_percentile(
            np.concatenate(
                (ez_xy_crop.ravel(), ez_xz_crop.ravel(), ez_yz_crop.ravel())
            ),
            field_pct,
        ),
        ez_floor_v_m,
    ...
    energy_vmax = weighted_percentile(
        tracked_energy, energy_pct, tracked_weights
    )
    ...
    selection_xy = select_publication_particle_plane(
        frame, tracked_ids, plane="xy", coordinate_m=center_z,
        slab_half_width_m=slab_half_width
    )
    selection_xz = select_publication_particle_plane(
        frame, tracked_ids, plane="xz", coordinate_m=center_y,
        slab_half_width_m=slab_half_width
    )
    selection_yz = select_publication_particle_plane(
        frame, tracked_ids, plane="yz", coordinate_m=center_x,
        slab_half_width_m=slab_half_width
    )
    ...
    fig.subplots_adjust(
        left=0.055, right=0.985, bottom=0.13, top=0.80, wspace=0.18
    )
    ...
    ax_xy.pcolormesh(
        ...
        ez_xy_matrix / 1.0e12,
        shading="flat", cmap="RdBu_r", vmin=ez_vmin, vmax=ez_vmax,
        rasterized=True,
    )
    ...
    qcolor_floor_t, qcolor_ceil_t = _positive_percentile_range(
        qmag, magnetic_visible_pct, magnetic_color_pct
    )
    denominator = max(
        qcolor_ceil_t - qcolor_floor_t,
        np.finfo(float).eps,
    )
    qnorm = np.clip((qmag - qcolor_floor_t) / denominator, 0.0, 1.0)
    qamp = np.power(qnorm, magnetic_gamma)
    display_mask = qnorm > 0.0
    safe_qmag = np.where(qmag > 0.0, qmag, 1.0)
    u_direction = np.divide(
        qbx, safe_qmag, out=np.zeros_like(qbx), where=qmag > 0.0
    )
    v_direction = np.divide(
        qby, safe_qmag, out=np.zeros_like(qby), where=qmag > 0.0
    )
    qu = np.ma.masked_where(~display_mask, u_direction * qamp)
    qv = np.ma.masked_where(~display_mask, v_direction * qamp)
    qc = np.ma.masked_where(~display_mask, qmag / 1.0e3)
    magnetic_quiver = ax_xy.quiver(
        qx_grid * 1.0e6,
        qy_grid * 1.0e6,
        qu,
        qv,
    ...
    magnetic_quiver.set_clim(
        qcolor_floor_t / 1.0e3,
        qcolor_ceil_t / 1.0e3,
    )

    panel_specs = (
        (ax_xy, background_xy_indices, tracked_xy_indices, x_m, y_m),
        (ax_xz, background_xz_indices, tracked_xz_indices, xi_particles, x_m),
        (ax_yz, background_yz_indices, tracked_yz_indices, xi_particles, y_m),
    )
    panel_marker_sizes: dict[str, float] = {}
    tracked_scatter = None
    for panel_name, spec in zip(("xy", "xz", "yz"), panel_specs):
        ax, background_indices, tracked_indices, horizontal, vertical = spec
        marker_size, background_alpha = _particle_scatter_style(
            int(background_indices.size)
        )
        panel_marker_sizes[panel_name] = float(marker_size)
        ax.scatter(
            horizontal[background_indices] * 1.0e6,
            vertical[background_indices] * 1.0e6,
            s=marker_size,
            c="0.18",
        ...
        tracked_scatter = ax.scatter(
            horizontal[tracked_indices] * 1.0e6,
            vertical[tracked_indices] * 1.0e6,
            s=marker_size,
    ...
    _top_colorbar(fig, ax_xy, magnetic_quiver, r"$|B_\perp|$ [kT]")
    _top_colorbar(fig, ax_xz, ez_xz_image, r"$E_z$ [TV/m]")
    _top_colorbar(
        fig, ax_yz, tracked_scatter,
    ...
    result = PublicationFig9SnapshotResult(
        ...
        magnetic_color_min_kT=qcolor_floor_t / 1.0e3,
        magnetic_color_max_kT=qcolor_ceil_t / 1.0e3,
        ...
        xy_vector_overlay="B/x,B/y inferno quiver",
        ...
        particle_marker_size_mode=(
            "tracked and background particles use the same marker size per panel"
        ),
        quiver_representation=(
            "inferno color encodes B_perp magnitude; arrows below the requested "
            "magnitude percentile are hidden and visible lengths scale nonlinearly"
        ),
    )
    ...
    manifest = {
        ...
        "xy_vector_overlay": "B/x,B/y inferno quiver",
        "magnetic_quiver_colormap": "inferno",
        "magnetic_visible_percentile": magnetic_visible_pct,
        "magnetic_color_percentile": magnetic_color_pct,
        "magnetic_length_gamma": magnetic_gamma,
        "particle_marker_size_mode": "same size for background and tracked",
        "panel_marker_sizes_points_squared": panel_marker_sizes,
        "xi_definition": (
